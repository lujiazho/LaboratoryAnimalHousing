<%@ page contentType="text/html;charset=UTF-8" language="java"  import="org.lah.AnimalBreeding.domain.EstrusSow" %>
...
<div class="x-nav">
      <span class="layui-breadcrumb">
        <a href="">首页</a>
        <a href="/LaboratoryAnimalHousing/findEstrusSow">母猪发情行为记录</a>
      </span>
</div>
<div class="x-body">
    <div class="layui-row">
        <form class="layui-form layui-col-md12 x-so" action="/LaboratoryAnimalHousing/findEstrusSow" >
    ...
    <xblock>
        <button id="addEstrusSowBtn" class="layui-btn"> <i class="layui-icon">&#xe654;</i>添加</button>
        <a class="layui-btn" href="/LaboratoryAnimalHousing/findEstrusSow" title="刷新"><i class="layui-icon">&#xe669;</i>刷新</a>
    ...
    <div class="layui-row" id="test" style="display: none;">
        <div class="layui-col-md10">
            <form class="layui-form" id="addEstrusSow" action="/LaboratoryAnimalHousing/addEstrusSow" method="post">
    ...
    <table class="layui-table">
        ...
        <tbody>
        <c:forEach items="${pi.list}" var="estrusSow">
        <tr>
            <td>${estrusSow.getActionID()}</td>
            <td>${estrusSow.getAnimalNumber()}</td>
            <td>${estrusSow.getBehaviorStartTime()}</td>
            <td>${estrusSow.getBehaviorEndTime()}</td>
            <td>${estrusSow.getBehaviorDescription()}</td>
            <td>${estrusSow.getTreatmentPlan()}</td>
            <td>${estrusSow.getTreatmentResult()}</td>
            <td>
                <a title="编辑"    id= "updateEdit"    href="/LaboratoryAnimalHousing/findEstrusSowById?ActionID=${estrusSow.getActionID()}">
                    <i class="layui-icon">&#xe642;</i>
                </a>
                <a title="删除" onclick="member_del(this,'${estrusSow.getActionID()}')" href="javascript:;">
                    <i class="layui-icon">&#xe640;</i>
                </a>
            </td>
        </tr>
...
<script>
    ...
    layui.use(['jquery', 'excel','form','layer','laydate'], function(){
        ...
        /*添加弹出框*/
        $("#addEstrusSowBtn").click(function () {
            layer.open({
                type:1,
                title:"添加母猪发情行为记录",
                skin:"myclass",
    ...
    function member_del(obj,ActionID){
        layer.confirm('确认要删除吗？',function(index){
            //发异步删除数据
           $.get("/LaboratoryAnimalHousing/deleteEstrusSow",{"ActionID":ActionID},function (data) {
                if(data =true){
                    layer.msg('删除成功!',{icon:1,time:2000});
                  setTimeout(function () {window.location.href='/LaboratoryAnimalHousing/findEstrusSow';},2000);
