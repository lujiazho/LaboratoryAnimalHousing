<%@ page contentType="text/html;charset=UTF-8" language="java"  import="org.lah.AnimalFeed.domain.FeedFeeding" %>
...
<div class="x-nav">
      <span class="layui-breadcrumb">
        <a href="">首页</a>
        <a href="/LaboratoryAnimalHousing/findFeedFeeding">查询饲料投喂信息</a>
      </span>
    <a class="layui-btn layui-btn-small" style="line-height:1.6em;margin-top:3px;float:right" href="/LaboratoryAnimalHousing/findFeedFeeding" title="刷新">
        <i class="layui-icon" style="line-height:30px">ဂ</i></a>
</div>
<div class="x-body">
    <div class="layui-row">
        <form class="layui-form layui-col-md12 x-so" action="/LaboratoryAnimalHousing/findFeedFeeding" >
            <input class="layui-input" placeholder="请输入投喂编号" name="FeedingNumber" id="FeedingNumber" value="${FeedingNumber}">
    ...
    <xblock>
        <button id="addFeedFeedingBtn" class="layui-btn layui-btn-normal"> <i class="layui-icon">&#xe654;</i>添加 </button>
    ...
    <div class="layui-row" id="test" style="display: none;">
        <div class="layui-col-md10">
            <form class="layui-form" id="addFeedFeeding" action="/LaboratoryAnimalHousing/addFeedFeeding" method="post">
                <div class="layui-form-item">
                    <label class="layui-form-label">投喂日期：</label>
                    <div class="layui-input-block">
                        <input type="text" name="FeedDate"  class="layui-input" placeholder="请输入投喂日期">
                    </div>
                </div>
                <div class="layui-form-item">
                ...
                <div class="layui-form-item">
                    <label class="layui-form-label">投喂房间号：</label>
                    <div class="layui-input-block">
                        <input type="text" name="RoomNumber" lay-verify="required" class="layui-input"  placeholder="请输入投喂房间号">
                    </div>
                </div>
                <div class="layui-form-item">
                <label class="layui-form-label">饲料数量：</label>
                <div class="layui-input-block">
                    <input type="text"  name="FeedAmount" lay-verify="required" class="layui-input" placeholder="请输入饲料数量">
                </div>
                </div>
                <div class="layui-form-item">
    ...
    <table class="layui-table">
        ...
        <tbody>
<c:forEach items="${pi.list}" var="feedFeeding">
        <tr>
            <%--<td>--%>
                <%--<div class="layui-unselect layui-form-checkbox" lay-skin="primary" data-id='2'><i class="layui-icon">&#xe605;</i></div>--%>
            <%--</td>--%>
            <td>${feedFeeding.getFeedingNumber()}</td>
            <td>
                <f:formatDate value="${feedFeeding.getFeedDate()}" type="date" dateStyle="long"/>
            </td>
            <td>${feedFeeding.getAnimalNumber()}</td>
            <td>${feedFeeding.getFeedType()}</td>
            <td>${feedFeeding.getRoomNumber()}</td>
            <td>${feedFeeding.getFeedAmount()}</td>
            <td>${feedFeeding.getPersonnelNumber()}</td>
            <td>${feedFeeding.getAbnormalCondition()}</td>
            <td>
                <a title="编辑"  onclick="return editpermission('${sessionScope.user_session.position}')"  id= "updateEdit"    href="/LaboratoryAnimalHousing/findFeedFeedingById?FeedingNumber=${feedFeeding.getFeedingNumber()}">
                    <i class="layui-icon">&#xe642;</i>
                </a>
                <a title="删除" onclick="member_del(this,'${feedFeeding.getFeedingNumber()}','${sessionScope.user_session.position}')" href="javascript:;">
                    <i class="layui-icon">&#xe640;</i>
                </a>
            </td>
        </tr>
...
<script>
    ...
    layui.use(['jquery', 'excel','form','layer','laydate'], function(){
        ...
        /*添加弹出框*/
        $("#addFeedFeedingBtn").click(function () {
    ...
    /*删除*/
    function member_del(obj,FeedingNumber,myposition){
        ...
        // }
            ...
            layer.confirm('确认要删除吗？', function (index) {
                //发异步删除数据
                $.get("/LaboratoryAnimalHousing/deleteFeedFeeding", {"FeedingNumber": FeedingNumber}, function (data) {
                    if (data = true) {
                        layer.msg('删除成功!', {icon: 1, time: 2000});
                        setTimeout(function () {
                            window.location.href = '/LaboratoryAnimalHousing/findFeedFeeding';
                        }, 2000);
