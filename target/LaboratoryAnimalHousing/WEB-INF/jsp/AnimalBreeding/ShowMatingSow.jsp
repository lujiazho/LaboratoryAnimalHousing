<%@ page contentType="text/html;charset=UTF-8" language="java"  import="org.lah.AnimalBreeding.domain.MatingSow" %>
...
<div class="x-nav">
      <span class="layui-breadcrumb">
        <a href="">首页</a>
        <a href="/LaboratoryAnimalHousing/findMatingSow">母猪配种行为记录</a>
      </span>
</div>
<div class="x-body">
    <div class="layui-row">
        <form class="layui-form layui-col-md12 x-so" action="/LaboratoryAnimalHousing/findMatingSow" >
    ...
    <xblock>
        <button id="addMatingSowBtn" class="layui-btn"> <i class="layui-icon">&#xe654;</i>添加</button>
        <a class="layui-btn" href="/LaboratoryAnimalHousing/findMatingSow" title="刷新"><i class="layui-icon">&#xe669;</i>刷新</a>
    ...
    <div class="layui-row" id="test" style="display: none;">
        <div class="layui-col-md10">
            <form class="layui-form" id="addMatingSow" action="/LaboratoryAnimalHousing/addMatingSow" method="post">
    ...
    <table class="layui-table">
        ...
        <tbody>
        <c:forEach items="${pi.list}" var="matingSow">
        <tr>
            <td>${matingSow.getActionID()}</td>
            <td>${matingSow.getAnimalNumber()}</td>
            <td>${matingSow.getBehaviorStartTime()}</td>
            <td>${matingSow.getBehaviorEndTime()}</td>
            <td>${matingSow.getBehaviorDescription()}</td>
            <td>${matingSow.getTreatmentPlan()}</td>
            <td>${matingSow.getTreatmentResult()}</td>
            <td>
                <a title="编辑"    id= "updateEdit"    href="/LaboratoryAnimalHousing/findMatingSowById?ActionID=${matingSow.getActionID()}">
                    <i class="layui-icon">&#xe642;</i>
                </a>
                <a title="删除" onclick="member_del(this,'${matingSow.getActionID()}')" href="javascript:;">
                    <i class="layui-icon">&#xe640;</i>
                </a>
            </td>
        </tr>
...
<script>
    ...
    layui.use(['jquery', 'excel','form','layer','laydate'], function(){
        ...
        /*添加弹出框*/
        $("#addMatingSowBtn").click(function () {
            layer.open({
                type:1,
                title:"添加母猪配种行为记录",
                skin:"myclass",
    ...
    function member_del(obj,ActionID){
        layer.confirm('确认要删除吗？',function(index){
            //发异步删除数据
           $.get("/LaboratoryAnimalHousing/deleteMatingSow",{"ActionID":ActionID},function (data) {
                if(data =true){
                    layer.msg('删除成功!',{icon:1,time:2000});
                  setTimeout(function () {window.location.href='/LaboratoryAnimalHousing/findMatingSow';},2000);
