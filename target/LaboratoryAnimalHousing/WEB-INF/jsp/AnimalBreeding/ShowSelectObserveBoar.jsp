<%@ page contentType="text/html;charset=UTF-8" language="java"  import="org.lah.AnimalBreeding.domain.SelectObserveBoar" %>
...
<div class="x-nav">
      <span class="layui-breadcrumb">
        <a href="">首页</a>
        <a href="/LaboratoryAnimalHousing/findSelectObserveBoar">种猪档案</a>
      </span>
</div>
<div class="x-body">
    <div class="layui-row">
        <form class="layui-form layui-col-md12 x-so" action="/LaboratoryAnimalHousing/findSelectObserveBoar" >
    ...
    <xblock>
<%--        <button id="addSelectObserveBoarBtn" class="layui-btn"> <i class="layui-icon">&#xe654;</i>添加</button>--%>
        <a class="layui-btn" href="/LaboratoryAnimalHousing/findSelectObserveBoar" title="刷新"><i class="layui-icon">&#xe669;</i>刷新</a>
    ...
    <table class="layui-table">
        ...
        <tbody>
        <c:forEach items="${pi.list}" var="SelectObserveBoar">
        <tr>
            <td>${SelectObserveBoar.getAnimalNumber()}</td>
            <td>${SelectObserveBoar.getPigAge()}</td>
            <td>${SelectObserveBoar.getFertility()}</td>
            <td>${SelectObserveBoar.getBreedingHistory()}</td>
            <td>
                <a title="设置为选种观察对象" onclick="member_del(this,'${SelectObserveBoar.getAnimalNumber()}')" href="javascript:;">
...
<script>
    ...
    function member_del(obj,AnimalNumber){
        layer.confirm('确认要设置为选种观察对象吗？',function(index){
            //发异步修改数据
           $.get("/LaboratoryAnimalHousing/updateSelectObserveBoar",{"AnimalNumber":AnimalNumber},function (data) {
                if(data =true){
                    layer.msg('初选成功!下一步请进行日常情况观察评估！',{icon:1,time:2000});
                  setTimeout(function () {window.location.href='/LaboratoryAnimalHousing/findSelectObserveBoar';},2000);

                }else {
                    layer.msg('初选失败!',{icon:1,time:3000});
                    setTimeout(function () {window.location.href='/LaboratoryAnimalHousing/findSelectObserveBoar';},2000);
                }
            });
        });
