<%@ page contentType="text/html;charset=UTF-8" language="java"  import="org.lah.AnimalBreeding.domain.SelectResultSow" %>
...
<div class="x-nav">
      <span class="layui-breadcrumb">
        <a href="">首页</a>
        <a href="/LaboratoryAnimalHousing/findSelectResultSow">母猪档案</a>
      </span>
</div>
<div class="x-body">
    <div class="layui-row">
        <form class="layui-form layui-col-md12 x-so" action="/LaboratoryAnimalHousing/findSelectResultSow" >
    ...
    <xblock>
<%--        <button id="addSelectResultSowBtn" class="layui-btn"> <i class="layui-icon">&#xe654;</i>添加</button>--%>
        <a class="layui-btn" href="/LaboratoryAnimalHousing/findSelectResultSow" title="刷新"><i class="layui-icon">&#xe669;</i>刷新</a>
    ...
    <table class="layui-table">
        ...
        <tbody>
        <c:forEach items="${pi.list}" var="SelectResultSow">
        <tr>
            <td>${SelectResultSow.getAnimalNumber()}</td>
            <td>${SelectResultSow.getPigAge()}</td>
            <td>${SelectResultSow.getHealthCondition()}</td>
            <td>${SelectResultSow.getLifeCondition()}</td>
            <td>
                <a title="设置为选种对象" onclick="member_del(this,'${SelectResultSow.getAnimalNumber()}')" href="javascript:;">
...
<script>
    ...
    function member_del(obj,AnimalNumber){
        layer.confirm('确认要设置为选种对象吗？',function(index){
            //发异步修改数据
           $.get("/LaboratoryAnimalHousing/updateSelectResultSow",{"AnimalNumber":AnimalNumber},function (data) {
                if(data =true){
                    layer.msg('终选成功!下一步请进行动物辅助配种！',{icon:1,time:2000});
                  setTimeout(function () {window.location.href='/LaboratoryAnimalHousing/findSelectResultSow';},2000);
