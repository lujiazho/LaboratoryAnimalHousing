<%@ page contentType="text/html;charset=UTF-8" language="java"  import="org.lah.AnimalBreeding.domain.MatingBoar" %>
...
<div class="x-nav">
      <span class="layui-breadcrumb">
        <a href="">首页</a>
        <a href="/LaboratoryAnimalHousing/findMatingBoar">种猪配种行为记录</a>
      </span>
</div>
<div class="x-body">
    <div class="layui-row">
        <form class="layui-form layui-col-md12 x-so" action="/LaboratoryAnimalHousing/findMatingBoar" >
    ...
    <xblock>
        <button id="addMatingBoarBtn" class="layui-btn"> <i class="layui-icon">&#xe654;</i>添加</button>
        <a class="layui-btn" href="/LaboratoryAnimalHousing/findMatingBoar" title="刷新"><i class="layui-icon">&#xe669;</i>刷新</a>
    ...
    <div class="layui-row" id="test" style="display: none;">
        <div class="layui-col-md10">
            <form class="layui-form" id="addMatingBoar" action="/LaboratoryAnimalHousing/addMatingBoar" method="post">
    ...
    <table class="layui-table">
        ...
        <tbody>
        <c:forEach items="${pi.list}" var="matingBoar">
        <tr>
            <td>${matingBoar.getActionID()}</td>
            <td>${matingBoar.getAnimalNumber()}</td>
            <td>${matingBoar.getBehaviorStartTime()}</td>
            <td>${matingBoar.getBehaviorEndTime()}</td>
            <td>${matingBoar.getBehaviorDescription()}</td>
            <td>${matingBoar.getTreatmentPlan()}</td>
            <td>${matingBoar.getTreatmentResult()}</td>
            <td>
                <a title="编辑"    id= "updateEdit"    href="/LaboratoryAnimalHousing/findMatingBoarById?ActionID=${matingBoar.getActionID()}">
                    <i class="layui-icon">&#xe642;</i>
                </a>
                <a title="删除" onclick="member_del(this,'${matingBoar.getActionID()}')" href="javascript:;">
                    <i class="layui-icon">&#xe640;</i>
                </a>
            </td>
        </tr>
...
<script>
    ...
    layui.use(['jquery', 'excel','form','layer','laydate'], function(){
        ...
        /*添加弹出框*/
        $("#addMatingBoarBtn").click(function () {
            layer.open({
                type:1,
                title:"添加种猪配种行为记录",
                skin:"myclass",
    ...
    function member_del(obj,ActionID){
        layer.confirm('确认要删除吗？',function(index){
            //发异步删除数据
           $.get("/LaboratoryAnimalHousing/deleteMatingBoar",{"ActionID":ActionID},function (data) {
                if(data =true){
                    layer.msg('删除成功!',{icon:1,time:2000});
                  setTimeout(function () {window.location.href='/LaboratoryAnimalHousing/findMatingBoar';},2000);
