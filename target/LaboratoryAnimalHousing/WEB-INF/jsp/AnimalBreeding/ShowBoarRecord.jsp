<%@ page contentType="text/html;charset=UTF-8" language="java"  import="org.lah.AnimalBreeding.domain.BoarRecord" %>
...
<div class="x-nav">
      <span class="layui-breadcrumb">
        <a href="">首页</a>
        <a href="/LaboratoryAnimalHousing/findBoarRecord">种猪档案</a>
      </span>
</div>
<div class="x-body">
    <div class="layui-row">
        <form class="layui-form layui-col-md12 x-so" action="/LaboratoryAnimalHousing/findBoarRecord" >
    ...
    <xblock>
        <a class="layui-btn" href="/LaboratoryAnimalHousing/findBoarRecord" title="刷新"><i class="layui-icon">&#xe669;</i>刷新</a>
    ...
    <table class="layui-table">
        ...
        <tbody>
        <c:forEach items="${pi.list}" var="boarRecord">
        <tr>
            <td>${boarRecord.getAnimalNumber()}</td>
            <td>${boarRecord.getPigAge()}</td>
            <td>${boarRecord.getHealthCondition()}</td>
            <td>${boarRecord.getLifeCondition()}</td>
            <td>${boarRecord.getFertility()}</td>
            <td>${boarRecord.getBreedingHistory()}</td>
            <td>
                <a title="添加繁育能力及历史" id= "updateEdit" href="/LaboratoryAnimalHousing/findBoarRecordById?AnimalNumber=${boarRecord.getAnimalNumber()}">
...
<script>
    ...
    function member_del(obj,AnimalNumber){
        layer.confirm('确认要删除吗？',function(index){
            //发异步删除数据
           $.get("/LaboratoryAnimalHousing/deleteBoarRecord",{"AnimalNumber":AnimalNumber},function (data) {
                if(data =true){
                    layer.msg('删除成功!',{icon:1,time:2000});
                  setTimeout(function () {window.location.href='/LaboratoryAnimalHousing/findBoarRecord';},2000);
